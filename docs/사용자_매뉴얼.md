...
7. [공통 유틸리티 (`utils.py`)](#7-공통-유틸리티)
8. [데이터 파일 규격](#8-데이터-파일-규격)
9. [DB 스키마](#9-db-스키마)
10. [VPS 배포 및 운영 가이드](#10-vps-배포-및-운영-가이드)
11. [유의사항 및 트러블슈팅](#11-유의사항-및-트러블슈팅)
...
> [!NOTE]
> 네이버 매물 수집(`collect_naver_listing.py`)에 대한 자세한 설명은 [네이버 매물데이터 인사이트 가이드](네이버_매물데이터_인사이트_가이드.md)를 참조하세요.
...
├── docs/                         # 문서
│   ├── 사용자_매뉴얼.md             # 본 매뉴얼
│   └── 네이버_매물데이터_인사이트_가이드.md  # 네이버 매물 전용 가이드
│
...
# 2. 네이버 매물 수집 (자세한 사용법은 네이버_매물데이터_인사이트_가이드.md 참조)
...
### 8.1 `apt_basic_info_master` — 아파트 기본 정보
...
### 8.2 `apt_detail_info_master` — 아파트 상세 정보
...
| `naver_complex` | `complex_no` | collect_naver_listing | 네이버 단지 정보 (행정동·위경도) |
| `naver_listing` | `article_no` | collect_naver_listing | 네이버 매물 (UPSERT 증분) |
...
### 10.1 최초 배포 (Hostinger VPS + Dokploy)
...
- **지오코딩**: 100건마다 중간 저장
- **네이버 매물**: `python collect_naver_listing.py daily --resume`으로 체크포인트에서 재개
